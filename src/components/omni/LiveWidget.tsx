...
function WidgetView({ type, data }: { type: string; data: any }) {
  ...
  switch (type) {
    ...
    case "spacex": return <div><p className="text-xs uppercase text-muted-foreground">Next launch</p><p className="mt-1 text-xl font-semibold">{data.name}</p><p className="mt-2 text-sm neon-text">{date(data.net)}</p><p className="mt-3 line-clamp-4 text-xs text-muted-foreground">{data.mission?.description || data.status?.description || "Mission details pending."}</p></div>;
