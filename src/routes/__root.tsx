    ...
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="glass max-w-md p-8 text-center">
        ...
        <div className="mt-6">
          <Link
            to="/"
            className="inline-flex items-center rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition hover:brightness-110"
          >
            Return to base
          </Link>
        </div>
...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="glass max-w-md p-8 text-center">
        <h1 className="text-xl font-semibold neon-text">System anomaly</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          The command center hit turbulence. Try again or head home.
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          ...
          <a
            href="/"
            className="inline-flex items-center rounded-lg border border-glass-border bg-glass px-4 py-2 text-sm font-medium transition hover:bg-secondary"
          >
            Home
          </a>
        </div>
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    ...
    links: [
      ...
      { rel: "icon", href: "/omnisphere-mark.svg", type: "image/svg+xml" },
    ],
  }),
      ...
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `try{var t=localStorage.getItem('omnisphere-theme')||'dark';document.documentElement.classList.toggle('dark',t==='dark');}catch(e){}`,
          }}
        />
      </head>
      <body>
